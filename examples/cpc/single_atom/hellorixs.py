...
def ed():
    ...
    emat_i[0:ndorb, 0:ndorb]       += atom_hsoc('d', zeta_d)
    emat_n[0:ndorb, 0:ndorb]       += atom_hsoc('d', zeta_d)
    emat_n[ndorb:ntot, ndorb:ntot] += atom_hsoc('p', zeta_p)
    ...
    for i in range(3):
        ...
        T_abs[i] = two_fermion(dipole[i], basis_n, basis_i)
        # Then, transfrom to the eigenvector basis
        T_abs[i] = np.dot(np.conj(np.transpose(evec_n)), 
                          np.dot(T_abs[i], evec_i)) 
...
def xas(eval_i, eval_n, T_abs): 
    ...
    prob = boltz_dist([eval_i[i] for i in gs], 300) 
    ...
    for i, om in enumerate(omega):
        for j in gs:
            ...
            xas[i] += prob[j]*np.sum(np.abs(F_mag)**2 * 
                      Gam_c/np.pi/((om-(eval_n[:]-eval_i[j]))**2 
                      + Gam_c**2))
...
def rixs(eval_i, eval_n, T_abs, T_emi):
    ...
    # Calculate RIXS
    for i, om in enumerate(omega):
        F_fi=scattering_mat(eval_i,eval_n,T_abs[:,:,gs],T_emi,
                           om,Gam_c)
        for j, (alpha, beta) in enumerate(pol):  
            ...
            for m in gs:
                for n in range(len(eval_i)):
                    rixs[j,:,i]+=(prob[m]*np.abs(F_mag[n,m])**2 
                    *Gam/np.pi/((eloss-(eval_i[n]-eval_i[m]))**2 
                                 +Gam**2 )) 
