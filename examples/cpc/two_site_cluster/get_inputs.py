...
from edrixs.basis_transform    import  cb_op, tmat_r2c, tmat_c2r, transform_utensor
...
def set_config():
    config=[
          ...
          "omega_in=-540.4    ! Relative incident x-ray energy at which RIXS calculations are performed",
          "&end"
       ]
...
def get_hopping_coulomb(locaxis):
    ...
    umat_t2gp_n = get_umat_slater('t2gp', *params)
    ...
    for m in range(2):
        for i in range(12):
            for j in range(12):
                for k in range(12):
                    for l in range(12):
                        umat_n[indx[m,i], indx[m,j], indx[m,k], indx[m,l]] += umat_t2gp_n[i,j,k,l]
    ...
    soc_d = atom_hsoc('t2g', zeta_d_i)
    soc_p = atom_hsoc('p', zeta_p_n)
    ...
    ang1, ang2, ang3 = rmat_to_euler(locaxis[0])
    dmat[0] = dmat_spinor(ang1, ang2, ang3)
    ang1, ang2, ang3 = rmat_to_euler(locaxis[1])
    ...
    t_orb[0:6,   0:6] = tmat_r2c('t2g', True)
    t_orb[6:12, 6:12] = tmat_r2c('t2g', True)
    crys_spin[:,:] = cb_op(crys_spin, np.dot(t_spinor, t_orb))
    ...
    # Write to files
    # ED inputs
    write_emat(emat_i, "ed/hopping_i.in")
    write_umat(umat_i, "ed/coulomb_i.in")

    # XAS inputs
    write_emat(emat_n, "xas/hopping_n.in")
    write_umat(umat_n, "xas/coulomb_n.in")
 
    # RIXS inputs
    write_emat(emat_i, "rixs_pp/hopping_i.in")
    write_umat(umat_i, "rixs_pp/coulomb_i.in")
    write_emat(emat_n, "rixs_pp/hopping_n.in")
    write_umat(umat_n, "rixs_pp/coulomb_n.in")

    write_emat(emat_i, "rixs_ps/hopping_i.in")
    write_umat(umat_i, "rixs_ps/coulomb_i.in")
    write_emat(emat_n, "rixs_ps/hopping_n.in")
    write_umat(umat_n, "rixs_ps/coulomb_n.in")

def get_fock_basis():
    write_fock_dec_by_N(12, 9,  'ed/fock_i.in')
...
def get_transop(loc, pos):
    ...
    kin, kout = get_wavevector_rixs(thin, thout, phi, ein, eout)
    ...
    for key, alpha, beta in [('pp', 0, 0), ('ps', 0, np.pi)]:
        ...
        for i in range(2):
            T_i[i] = (dop_g[i,0]*ei[0] + dop_g[i,1]*ei[1] + dop_g[i,2]*ei[2])*np.exp(1j*np.dot(pos[i], kin))
            T_o[i] = np.conj(np.transpose(dop_g[i,0]*ef[0] + dop_g[i,1]*ef[1] + dop_g[i,2]*ef[2]))*np.exp(-1j*np.dot(pos[i], kout))
        ...
        write_emat(transop_rixs_i, "rixs_"+key+"/transop_rixs_i.in")  
        write_emat(transop_rixs_f, "rixs_"+key+"/transop_rixs_f.in")  
    ...
    write_emat(transop_xas, "xas/transop_xas.in")  
