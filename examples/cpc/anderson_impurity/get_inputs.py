...
def get_hopping_coulomb():
    ...
    umat_i = get_umat_slater('t2g', *params)
    ...
    umat_tmp = get_umat_slater('t2gp', *params)
    tmat = np.zeros((12,12), dtype=np.complex128)
    tmat[0:6,0:6] = tmat_c2j(1)
    tmat[6:12,6:12] = tmat_c2j(1)
    umat_i[:,:,:,:] = transform_utensor(umat_i, tmat_c2j(1))
    ...
    for i in range(10):
        for j in range(10):
            for k in range(10):
                for l in range(10):
                    umat_n[id2[i],id2[j],id2[k],id2[l]] = umat_tmp[id1[i],id1[j],id1[k],id1[l]]
    ...
    for i in range(N_site):
        ...
        h[2,o+2]=h[3,o+3]=h[4,o+4]=h[5,o+5]=h[o+2,2]=h[o+3,3]=h[o+4,4]=h[o+5,5]=v2[i]
    ...
    # Write to files
    # Search GS inputs
    write_emat(emat_i, "search_gs/hopping_i.in")
    write_umat(umat_i, "search_gs/coulomb_i.in")
...
def get_transop():
    ...
    for i in range(3):
        tmp[i] = np.dot( np.conj(np.transpose(tmat_c2j(1))), np.dot(tmp[i], tmat_c2j(1)) )
    ...
    for key, alpha, beta in [('pp', 0, 0), ('ps', 0, np.pi)]:
        ...
        transop_rixs_f = np.conj(np.transpose(dipole[0] * ef[0] + dipole[1] * ef[1] + dipole[2] * ef[2]))
