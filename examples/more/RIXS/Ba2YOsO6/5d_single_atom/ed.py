...
if __name__ == "__main__":
    ...
    F0_dp = Udp + get_F0('dp', G1_dp, G3_dp)
    ...
    # 5d, with core-hole
    zeta_d_n = 0.33
    # 2p core electron, and usually we will adjust it to match the energy difference of the two edges,
    ...
    basis_i = get_fock_bin_by_N(ndorb, 3, nporb, nporb)
    ncfgs_i = len(basis_i)
    
    # build Fock basis, with a core-hole
    # 9 5d-electrons and 5 2p-electrons
    basis_n = get_fock_bin_by_N(ndorb, 4, nporb, nporb-1)
    ncfgs_n = len(basis_n)
