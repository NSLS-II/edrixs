...
if __name__ == '__main__':
    ...
    trans_mat_abs = data[:,4].reshape((2, 3, ncfgs_ex, ncfgs_gs)) + 1j * data[:,5].reshape((2,3, ncfgs_ex, ncfgs_gs))
    ...
    omi_mesh = np.linspace(-540, -540, 1)
    ...
    for omi_ind, omi in enumerate(omi_mesh):
        ...
        for i in range(0,2):
            ...
            Ffg_Ir[i]=scattering_mat(eval_gs, eval_ex[i], abs_trans[:,:,gs_list], emi_trans, omi, gamma_c)
        print('------------------------')
        for ipol, (alpha,beta) in enumerate(pol_vec):
            ein, eout = omi+omi_shift, omi+omi_shift
            ei, ef =  dipole_polvec_rixs(thin, thout, phi, alpha, beta, local_axis=np.eye(3))
            kin, kout = get_wavevector_rixs(thin, thout, phi, ein, eout, local_axis=np.eye(3))
            Q = kout - kin
    ...
    for i in range(len(eloss_mesh)):
        for j in range(len(omi_mesh)):
            line="{:20.10f}{:20.10f}{:20.10f}{:20.10f}\n".format(eloss_mesh[i], omi_mesh[j]+omi_shift, rixs[0,i,j], rixs[1,i,j]) 
            f.write(line)
    f.close()
