...
from edrixs.basis_transform    import  cb_op, cb_op2, tmat_r2c, tmat_c2r, transform_utensor
...
if __name__ == "__main__":
    ...
    umat_t2g_i = transform_utensor(umat_t2g_i, tmat_c2r('t2g', True)) 
    params=[
    ...
    umat_t2gp_n = transform_utensor(umat_t2gp_n, tmat)
    ...
    for m in range(2):
        for i in range(12):
            for j in range(12):
                for k in range(12):
                    for l in range(12):
                        umat_n[m, indx[m,i], indx[m,j], indx[m,k], indx[m,l]] += umat_t2gp_n[i,j,k,l]

    emat_i = np.zeros((norbs,norbs), dtype=np.complex128)
    emat_n = np.zeros((2,norbs,norbs), dtype=np.complex128)
 
    soc_d = cb_op(atom_hsoc('t2g', zeta_d_i), tmat_c2r('t2g', True))
    soc_p = cb_op(atom_hsoc('p', zeta_p_n), tmat_c2r('p', True))
    ...
    ang1, ang2, ang3 = rmat_to_euler(np.transpose(Ir_loc2g[0]))
    dmat[0] = dmat_spinor(ang1, ang2, ang3)
    ang1, ang2, ang3 = rmat_to_euler(np.transpose(Ir_loc2g[1]))
    ...
    crys_spin[:,:] = cb_op(crys_spin, t_spinor)
    ...
    basis_i = get_fock_bin_by_N(12, 9, 12, 12)
    basis_n = []
    tmp = get_fock_bin_by_N(12, 10, 6, 5, 6, 6)
    basis_n.append(tmp)
    tmp = get_fock_bin_by_N(12, 10, 6, 6, 6, 5)
    basis_n.append(tmp)
    ...
    for i in range(2):
        hmat_n[i] += two_fermion(emat_n[i],  basis_n[i], basis_n[i])
        hmat_n[i] += four_fermion(umat_n[i], basis_n[i])
    ...
    tran_ptod = get_trans_oper('t2gp')
    for i in range(3):
        tran_ptod[i,:,:] = cb_op2(tran_ptod[i], tmat_c2r('t2g',True), tmat_c2r('p',True))
    ...
    for i in range(2):
        for j in range(3):
            trop_n[i,j] = two_fermion(dipole_n[i,j], basis_n[i], basis_i)
            trop_n[i,j] = cb_op2(trop_n[i,j], evec_n[i], evec_i)

    Ir_trop = np.zeros((2, 3, ncfgs_n, ncfgs_i), dtype=np.complex128)
    for i in range(2):
        for j in range(3):
            Ir_trop[i,j] = Ir_loc2g[i,0,j]*trop_n[i,0] + Ir_loc2g[i,1,j]*trop_n[i,1] + Ir_loc2g[i,2,j]*trop_n[i,2]
    write_tensor(Ir_trop, 'trans_mat.dat')
    print("edrixs >>> Done !")
