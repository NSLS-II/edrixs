...
if __name__ == "__main__":
    ...
    Purpose: This is a "Hello World !" case for RIXS simulation.
             This example shows how to exactly diagonalize a Hamiltonian for valence electrons
             and for valence + core electrons,
    ...
    # here, we are performing Ni-L2, L3 edges X-ray scattering,
    # which means Ni-2p --> Ni-3d transitions, so we have two atomic shells:
    # one is the valence shell 3d(l=2) and another is core-electron
    # shell 2p(l=1). Here, we use a convention that the orbitals of valence shells
    # are always in front of that of the core-electron shells.
    # For this case, we work on the complex spherical harmonics basis, so the orbital order will be:
    # |2,-2,up> |2,-2,dn>, |2,-1,up>, |2,-1,dn>, ..., |2,2,up>, |2,2,dn>
    # |1,-1,up>, |1,-1,dn>, ..., |1,1,up>, |1,1,dn>
    ...
    # Here, Ud_av is the averaged Coulomb interaction in 3d shell,
    # we set it to be zero for simplicity.
    ...
    zeta_d_n = 0.102
    # for 2p core electron, from Cowan's code, and usually we will adjust it to match the
    # energy difference of the two edges,
    # for example, roughly, E(L2-edge) - E(L3-edge) = 1.5 * zeta_p_n
    zeta_p_n = 11.24

    # Crystal Field
    # For this case, it has a tetrgonal symmetry, see
    # [https://doi.org/10.1016/j.ccr.2004.03.018] for details.
    # cubic splitting
    dq10 = 1.3
    ...
    # NOTE: the RIXS scattering involves 2 process, the absorption and emission,
    # so we have 3 states:
