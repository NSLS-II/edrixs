...
if __name__ == "__main__":
    ...
    for m, ei in enumerate(polvec):
        for i, om_inc in enumerate(om_mesh):
            for j in gs_list:
                scatt_mat = trans_mat[0, :, j] * ei[0] + \
                    trans_mat[1, :, j] * ei[1] + trans_mat[2, :, j] * ei[2]
                xas[m, i] += np.sum(np.abs(scatt_mat)**2 * gamma / np.pi /
                                    ((om_inc - (eval_n[:] - eval_i[j]))**2 + gamma**2)) * gs_prob[j]
    print("edrixs >>> Done !")

    f = open('xas.dat', 'w')
    f.write("#       omega_inc              pi-pol             sigma-pol           left-pol \
            right-pol          isotropic \n")
    for i in range(nom):
        f.write("{:20.10f}{:20.10f}{:20.10f}{:20.10f}{:20.10f}{:20.10f}\n".format(
            om_offset + om_mesh[i], xas[0, i], xas[1, i], xas[2, i], xas[3, i], xas[4, i]))
    f.close()
