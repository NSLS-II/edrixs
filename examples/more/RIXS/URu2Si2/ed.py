...
if __name__ == "__main__":
    ...
    # (2) after the absorption, a core hole is created in the core shell, both the valence and
    # core shells are involved, we use the letter (n) to label it
    ...
    tmat[0:nforb, 0:nforb] = edrixs.tmat_c2j(3)
    tmat[nforb:norbs, nforb:norbs] = edrixs.tmat_c2j(2)
    ...
    umat_tmp = edrixs.get_umat_slater('fd', *params)
    umat_i = edrixs.transform_utensor(umat_tmp, tmat)
    ...
    umat_tmp = edrixs.get_umat_slater('fd', *params)
    umat_n = edrixs.transform_utensor(umat_tmp, tmat)

    # build SOC matrix
    hsoc_i = np.zeros((norbs, norbs), dtype=np.complex128)
    hsoc_n = np.zeros((norbs, norbs), dtype=np.complex128)
    hsoc_i[0:nforb, 0:nforb] += edrixs.cb_op(edrixs.atom_hsoc('f', zeta_f_i), edrixs.tmat_c2j(3))
    hsoc_n[0:nforb, 0:nforb] += edrixs.cb_op(edrixs.atom_hsoc('f', zeta_f_n), edrixs.tmat_c2j(3))
    hsoc_n[nforb:norbs,
           nforb:norbs] += edrixs.cb_op(edrixs.atom_hsoc('d', zeta_d_n), edrixs.tmat_c2j(2))

    # build Fock basis, without core-hole
    # 2 5f-electrons and 10 5d-electrons
    basis_i = edrixs.get_fock_bin_by_N(nforb, 2, ndorb, ndorb)
    ncfgs_i = len(basis_i)

    # build Fock basis, with a core-hole
    # 3 5f-electrons and 9 5d-electrons
    basis_n = edrixs.get_fock_bin_by_N(nforb, 3, ndorb, ndorb - 1)
    ncfgs_n = len(basis_n)
    ...
    tmp = edrixs.get_trans_oper('fd')
    for i in range(3):
        dipole[i, 0:nforb, nforb:norbs] = edrixs.cb_op2(
            tmp[i], edrixs.tmat_c2j(3), edrixs.tmat_c2j(2))
