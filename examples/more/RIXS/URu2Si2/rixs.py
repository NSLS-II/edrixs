...
if __name__ == "__main__":
    ...
    Purpose: This example shows how to calculate RIXS spectrum.
             RIXS(eloss, om_inc) = \sum_{fi} ( |\sum_{mn} e_{f}^{m} *F_{fi}^{mn} * e_{i}^{n}|^2 * Gamma/pi / [ (eloss - (E_f-E_i))^2 + Gamma^2 ] * Prob[i] ), 
    ...
    for i, om_inc in enumerate(om_mesh):
        ...
        F_fi = scattering_mat(eval_i, eval_n, trans_mat_abs[:,:,gs_list], trans_mat_emi, om_inc, gamma[i])
