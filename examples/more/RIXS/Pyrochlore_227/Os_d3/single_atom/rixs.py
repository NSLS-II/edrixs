...
if __name__ == '__main__':
    ...
    gamma_c, gamma = 5.0 / 2.0, 0.13
    phi, thin, thout = 0.0, 56 / 180.0 * np.pi, 34 / 180.0 * np.pi
    pol_vec = [(0, 0), (0, np.pi / 2.0)]
    xx = [-1, 0, 0]
    zz = [0, 1 / np.sqrt(2.0), 1 / np.sqrt(2.0)]
    yy = np.cross(zz, xx)
    loc_scatt = np.transpose(np.array([xx, yy, zz]))

    omi_shift = 11371.0
    omi_mesh = np.linspace(-504, -496, 100)
    eloss_mesh = np.linspace(-0.1, 2, 1000)
    gs_list = range(0, 4)
    gs_dist = edrixs.boltz_dist([eval_gs[i] for i in gs_list], 300)
    ...
    plt.plot(eloss_mesh, [10.871] * len(eloss_mesh), '--', color='white')
