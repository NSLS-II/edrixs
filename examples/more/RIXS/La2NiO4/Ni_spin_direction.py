...
def do_ed(dq10=1.6, d1=0.1, d3=0.75,
    ...
    # They can be calculated by, for example, the Cowan's code:
    # [https://www.tcd.ie/Physics/people/Cormac.McGuinness/Cowan/]
    # by using Hartree-Fock mean-field method, and then are re-scaled, for example,
    # Fdd: 65%, Fdp: 95%, Gdp: 70%
    ...
    # Here, Ud_av is the averaged Coulomb interaction in 3d shell, we set it to be zero
    # for simplicity.
    ...
    zeta_d_n = 0.102
    # for 2p core electron, from Cowan's code, and usually we will adjust it to match the energy
    ...
    # NOTE: the RIXS scattering involves 2 process, absorption and emission, so we have 3 states:
    # (1) before the absorption, only valence shells are involved, we use the letter (i) to label it
    # (2) after the absorption, a core hole is created in the core shell, both the valence and core
    # shells are involved, we use the letter (n) to label it
    # (3) after the emission, the core hole is re-filled by an electron, we use the letter (f) to
    # label it.
    ...
    # the d-orbitals
    exchange = ex_x * edrixs.get_sx(2) + ex_y * edrixs.get_sy(2) + ex_z * edrixs.get_sz(2)
...
def get_xas(eval_i, eval_n, dipole_ops,
    ...
    # adjust om_offset to match the experimental X-ray energy, it can be roughly estimated by,
    # for example, Ni-L2/L3 edge, om_offset = (2*E_L3 + E_L2)/3, E_L3 and E_L2 can be found in
    # software, such as, Hephaestus [https://bruceravel.github.io/demeter/aug/hephaestus.html].
    # for Ni, E_L3=852.7 eV and E_L2=870 eV, so the calculated om_offset is 858.47 eV, however,
    # due to the different crystal environment, we fine tune it to be 857.40 according to the
    # experimental XAS and RIXS data.
    ...
    pol_pi = edrixs.dipole_polvec_xas(thin, phi, 0)
    # (1) linear \sigma-pol, the polarization vector is normal to the scattering plane
    pol_sigma = edrixs.dipole_polvec_xas(thin, phi, np.pi / 2.0)
    ...
    for m, ei in enumerate(polvec):
        for i, om_inc in enumerate(om_mesh):
            for j in gs_list:
                scatt_mat = dipole_op[0, :, j] * ei[0] + \
                    dipole_op[1, :, j] * ei[1] + dipole_op[2, :, j] * ei[2]
                xas[m, i] += np.sum(np.abs(scatt_mat)**2 * gamma / np.pi /
                                    ((om_inc - (eval_n[:] - eval_i[j]))**2 + gamma**2)) * gs_prob[j]
...
def get_rixs(eval_i, eval_n, dipole_ops,
    ...
    for i, om_inc in enumerate(om_mesh):
        F_fi = edrixs.scattering_mat(
            eval_i, eval_n, trans_mat_abs[:, :, gs_list], trans_mat_emi, om_inc, gamma_n)
