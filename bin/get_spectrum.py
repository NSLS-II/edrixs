...
if __name__ == "__main__":
    ...
    parser.add_argument('-f',      type=str,   default='spectrum.dat', help='File to store the calculated spectrum')
    parser.add_argument('files_of_poles', type=str, nargs='+', help='Pole-files generated by xas.x or rixs.x: xas_poles.xxx, rixs_poles.xxx')

    args = parser.parse_args()
    omega_mesh=np.linspace(args.ommin, args.ommax, args.N)
    gamma_mesh=np.ones(args.N) * args.G
    plot_spectrum(args.files_of_poles, omega_mesh, gamma_mesh, args.T, args.f, om_shift=args.off)
