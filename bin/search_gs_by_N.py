...
if __name__ == "__main__":
    ...
    parser.add_argument('-maxiter',  type=int,  default=1000, help='Maximum iterations of Lanczos process')
    parser.add_argument('-tol',  type=float,  default=1E-10, help='Tolerance for eigenvalue of ground state')
    parser.add_argument('-mpi_cmd',  type=str,  default='mpi_cmd.dat', help='File containing the MPI command')
    ...
    for occu in range(occu_start, occu_end+1):
        ndim=write_fock_dec_by_N(Norb, occu, 'fock_i.in')
        ...
        den=data[:,3].reshape((nvector, Norb, Norb)) + 1j * data[:,4].reshape((nvector, Norb, Norb))
